...
import { CATS } from "@/components/data/ecosystem";
import type { EcoCategory } from "@/components/types/sections/ecosystem";
...
interface EcosystemSectionProps {
  onReady: (
    flipTo: (nextIdx: number, direction: number) => void,
    getIdx: () => number,
    getFlipping: () => boolean
  ) => void;
}
...
export function EcosystemSection({ onReady }: EcosystemSectionProps) {
  ...
  function flipTo(nextIdx: number, direction: number) {
    ...
    if (isMobile) {
      ...
      if (curCat)  gsap.to(curCat, {
        rotationY: direction > 0 ? -90 : 90, z: -60, opacity: 0,
        duration: DUR, ease: "power3.inOut", transformOrigin: "50% 50%",
      });
      if (curLogo) gsap.to(curLogo, {
        rotationY: direction > 0 ?  90 : -90, z: -60, opacity: 0,
        duration: DUR, ease: "power3.inOut", transformOrigin: "50% 50%",
      });
      if (nxtCat)  gsap.to(nxtCat, {
        rotationY: 0, z: 0, opacity: 1,
        duration: DUR, ease: "power3.inOut", delay: 0.05, transformOrigin: "50% 50%",
        onComplete: () => { ecoFlippingRef.current = false; },
      });
      if (nxtLogo) gsap.to(nxtLogo, {
        rotationY: 0, z: 0, opacity: 1,
        duration: DUR, ease: "power3.inOut", delay: 0.05, transformOrigin: "50% 50%",
      });
    } else {
      ...
      if (curCat)  gsap.to(curCat, {
        rotationX: direction > 0 ? -90 : 90, z: -60, opacity: 0,
        duration: DUR, ease: "power3.inOut", transformOrigin: "50% 0%",
      });
      if (curLogo) gsap.to(curLogo, {
        rotationX: direction > 0 ? 90 : -90, z: -60, opacity: 0,
        duration: DUR, ease: "power3.inOut", transformOrigin: "50% 100%",
      });
      if (nxtCat)  gsap.to(nxtCat, {
        rotationX: 0, z: 0, opacity: 1,
        duration: DUR, ease: "power3.inOut", delay: 0.05, transformOrigin: "50% 100%",
        onComplete: () => { ecoFlippingRef.current = false; },
      });
      if (nxtLogo) gsap.to(nxtLogo, {
        rotationX: 0, z: 0, opacity: 1,
        duration: DUR, ease: "power3.inOut", delay: 0.05, transformOrigin: "50% 0%",
      });
    }
  }
  ...
  function handleReady(el: HTMLDivElement | null) {
    ...
    onReady(flipTo, () => ecoIdxRef.current, () => ecoFlippingRef.current);
  }
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div id="eco-left" style={{ position: "relative", overflow: "hidden", transformStyle: "preserve-3d", borderRight: "1px solid rgba(200,169,110,.08)" }}>
          {CATS.map((cat, i) => (
            <div
              key={cat.id}
              ref={(el) => { leftRefs.current[i] = el; }}
              className="eco-face eco-cat-face"
              style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", justifyContent: "center", padding: "80px 64px 80px 72px", overflow: "hidden", backfaceVisibility: "hidden" }}
            >
              ...
              <div style={{ position: "relative", zIndex: 1 }}>
                ...
                <div className="eco-cat-stat">
                  ...
                  <div className="eco-cat-stat-label" style={{ whiteSpace: "pre-line" }}>{cat.statLabel}</div>
                </div>
              </div>
            </div>
          ))}
        ...
        <div id="eco-right" style={{ position: "relative", overflow: "hidden", transformStyle: "preserve-3d" }}>
          {CATS.map((cat, i) => (
            <div
              ...
              style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "60px 52px", overflow: "hidden", backfaceVisibility: "hidden" }}
            >
      ...
      >
        {CATS.map((cat, i) => (
          <div
            ...
            onClick={() => { if (i !== ecoIdxRef.current) flipTo(i, i > ecoIdxRef.current ? 1 : -1); }}
          >
      ...
      <div
        ...
        style={{
          ...
          fontFamily: "var(--f-sans)", fontSize: 8, letterSpacing: ".26em", textTransform: "uppercase",
          color: "rgba(200,169,110,.28)", pointerEvents: "none", whiteSpace: "nowrap",
        }}
      >
