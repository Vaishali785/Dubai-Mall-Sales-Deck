import { BRAND_TILES, TICKER_BRANDS } from "@/components/data/retailBrands";
import type { BrandTile } from "@/components/types/sections/retailBrands";
...
function BrandTile({ tile }: { tile: BrandTile }) {
  ...
  return (
    <div
      ...
      style={tile.isHero ? { gridRow: "span 2" } : undefined}
    >
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{
          height: 46, padding: "10px 52px 0", zIndex: 20, opacity: 0,
          background: "linear-gradient(to top,rgba(6,5,4,.9) 0%,transparent 100%)",
        }}
      >
        <div style={{
          fontFamily: "var(--f-sans)", fontSize: 8, letterSpacing: ".22em", textTransform: "uppercase",
          color: "rgba(200,169,110,.4)", fontWeight: 300, marginRight: 24, flexShrink: 0,
        }}>
          Also at The Mall
        </div>
        <div className="brands-ticker-mask" style={{ display: "flex", overflow: "hidden", flex: 1, maskImage: "linear-gradient(to right,transparent 0%,black 5%,black 95%,transparent 100%)" }}>
          <div className="brands-inner" style={{ display: "flex", gap: 24, alignItems: "center", animation: "tickerScroll 30s linear infinite", whiteSpace: "nowrap" }}>
            {doubled.map((brand, i) => (
              <span key={i} style={{ fontFamily: "var(--f-serif)", fontSize: 11, fontStyle: "italic", color: "rgba(240,235,225,.28)", flexShrink: 0 }}>
                {brand}
              </span>
            ))}
          </div>
